...
# ---------------------------------------------------------------------------
# Optimizer (frozen — identical to train.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Evaluation on language-specific eval set
# ---------------------------------------------------------------------------

EVAL_TOKENS = 40 * 524288

@torch.no_grad()
def evaluate_bpb_filtered(model, tokenizer, batch_size, lang):
    """BPB evaluation on the language-specific eval set."""
    token_bytes = get_token_bytes(device="cuda")
    eval_loader = make_eval_dataloader(tokenizer, batch_size, MAX_SEQ_LEN, lang)
    steps = EVAL_TOKENS // (batch_size * MAX_SEQ_LEN)
    total_nats = 0.0
    total_bytes = 0
    for _ in range(steps):
        x, y, _ = next(eval_loader)
...
print(f"Loading filtered data for {LANG}...")
train_loader = make_filtered_dataloader(tokenizer, DEVICE_BATCH_SIZE, MAX_SEQ_LEN, LANG)
...
# Final eval on language-specific eval set
model.eval()
with autocast_ctx:
    val_bpb = evaluate_bpb_filtered(model, tokenizer, DEVICE_BATCH_SIZE, LANG)
